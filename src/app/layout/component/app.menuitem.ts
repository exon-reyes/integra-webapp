...
@Component({
    ...
    template: `
        <ng-container>
            ...
            <!-- 1. Texto raíz visible -->
            @if (root && item.visible !== false) {
                <div class="layout-menuitem-root-text">{{ item.label }}</div>
            }

            <!-- 2. Enlace sin routerLink o con sub-items -->
            ...
            <!-- 3. Enlace final con routerLink y sin sub-items -->
            @if (item.routerLink && !item.items && item.visible !== false) {
                <a
                    (click)="itemClick($event)"
                    [ngClass]="item.styleClass"
            ...
            <!-- 4. Submenús recursivos -->
            @if (item.items && item.visible !== false) {
                <ul [@children]="submenuAnimation">
                    @for (child of item.items; track child.id; let i = $index) {
